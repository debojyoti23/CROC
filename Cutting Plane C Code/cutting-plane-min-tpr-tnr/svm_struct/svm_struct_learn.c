			    ...
			    STRUCTMODEL *sm, int alg_type)
{
  ...
  */
  ...
  /*-------timing statistics------*/
  FILE *fop_time = fopen(sparm->timestatsfile, "w");  
  int wtdim;
  /*******/
  ...
  /* ADDED BY HARIKRISHNA */
	fprintf(fop_time, "%lf ", rt_total/100);
	for(wtdim = 1; wtdim < svmModel->totwords; wtdim++) // totwords - 1 = no. of features (last component is 0)
		fprintf(fop_time, "%lf ", svmModel->lin_weights[wtdim]);
	fprintf(fop_time, "\n");
	// print time, model vector
    	// divide by 100 to convert to secs
  /******/
  ...
  do { /* iteratively find and add constraints to working set */
	...
	printf("(NumConst=%d, SV=%ld, CEps=%.4f, QPEps=%.4f)\n",cset.m,
      ...
      /* ADDED BY HARIKRISHNA */
	fprintf(fop_time, "%lf ", rt_total/100);
        for(wtdim = 1; wtdim < svmModel->totwords; wtdim++) // svmModel->totwords -1 = no. of features (last component 0)
		fprintf(fop_time, "%lf ", svmModel->lin_weights[wtdim]);
	fprintf(fop_time, "\n");
	// print time, model vector
	// divide by 100 to convert to secs
