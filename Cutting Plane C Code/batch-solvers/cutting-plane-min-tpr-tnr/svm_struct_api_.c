...
//         sample.examples[0].y.class[i] = 100.0/(numn+nump);
      else
        sample.examples[0].y.class[i]=-0.5*100.0/(numn+nump);
//         sample.examples[0].y.class[i] = -100.0/(numn+nump);
    }
						     ...
						     int loss_type)
{
  ...
  start=1;
  /* Modified by Harikrishna */
  //prec_rec_k=(int)(nump*sparm->prec_rec_k_fr+++ac);
  prec_rec_k=(int)ceil(nump*sparm->prec_rec_k_frac);
  /****/
			       ...
			       STRUCT_LEARN_PARM *sparm)
{
  ...
  // Save output weight vector to 'file'
  fop = fopen(file, "w");
  for(j = 1; j <= sm->svm_model->totwords; j++) // totwords = no. of features
	fprintf(fop, "%lf\n", sm->w[j]);
  fclose(fop);*/
...
void         parse_struct_parameters(STRUCT_LEARN_PARM *sparm)
{
  ...
  for(i=0;(i<sparm->custom_argc) && ((sparm->custom_argv[i])[0] == '-');i++) {
    switch ((sparm->custom_argv[i])[2]) 
      ...
      /* ----------- ADDED BY HARIKRISHNA ---------------- */
      case 'm': i++; strcpy(sparm->timestatsfile, sparm->custom_argv[i]); 
      break;
      /* ------------------------------------------------- */
